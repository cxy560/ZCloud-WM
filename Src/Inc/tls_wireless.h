...
struct tls_wif {
    ...
    int (*rx_data_cb)(u8 *buf, u32 buf_len);
...
int tls_wl_if_set_bssid(struct tls_wif *wif, const u8 *addr);
...
int tls_wl_if_send_eapol(struct tls_wif *wif,
        u8 *buf, u32 len);
int tls_wl_if_xmit(struct tls_wif *wif, void *buf, int len);
...
int tls_wl_if_tx(struct tls_wif *wif,
        u8 *buf, u32 buflen, bool last_packet);
...
int tls_wl_if_standby( int type,
        int delay, int wake_time);
